...
main(int argc, char **argv)
{
  ...
  printf("Open files and then try to open them twice\n");
  for (i = 0; i < 2; i++) {
    names[i] = rand_name();
  ...
  printf("Two files created with zero length. Now writing\n");
  ...
  printf("Closing already closed files\n");
  if (sfs_fclose(fds[1]) == 0) {
   ...
   */
  printf("attempt to read from closed file\n");
   ...
   */
   printf("closing already closed file\n");
  for (i = 0; i < 2; i++) {
    if (sfs_fclose(fds[i]) == 0) {
   ...
   */
     printf("OPening a bunch of files\n");
  ncreate = 0;
  for (i = 0; i < MAX_FD; i++) {
    names[i] = rand_name();
    fds[i] = sfs_fopen(names[i]);
